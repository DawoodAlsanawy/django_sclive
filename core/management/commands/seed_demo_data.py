...
class Command(BaseCommand):
    ...
    def create_demo_invoices(self, num_invoices, sick_leaves, companion_leaves, force):
        ...
        for i, (leave_type, leave_id, duration_days) in enumerate(selected_leaves):
            ...
            # تحديد المبلغ بناءً على نوع الإجازة ومدتها والعميل
            try:
                # البحث عن سعر ثابت مخصص للعميل
                price_obj = LeavePrice.objects.filter(
                ...
                if price_obj:
                    if price_obj.pricing_type == 'per_day':
                        price = price_obj.price
                    else:  # fixed price
                        price = price_obj.price
                else:
                    raise LeavePrice.DoesNotExist
